...
# 解压
tar -zxvf mongodb-linux-x86_64-ubuntu1804-5.0.14.tgz

# 重命名
mv mongodb-linux-x86_64-ubuntu1804-5.0.14 mongodb
```
...
1. 日志目录是否存在：确保 /data/log/ 目录已经创建，否则启动会报错。可以执行 `mkdir -p /data/log`。

2. 配置文件启动（推荐）：直接敲一长串命令容易出错，建议写成配置文件：创建一个配置文件：

```bash
cat > /usr/local/mongodb/mongodb/mongod.conf << EOF
dbpath=/data
...
```bash
/usr/local/mongodb/mongodb/bin/mongod -f /usr/local/mongodb/mongodb/mongod.conf
```

3. 环境变量（可选）：为了不用每次都输绝对路径，可以把 bin 目录加到环境变量里：

```bash
echo 'export PATH=$PATH:/usr/local/mongodb/mongodb/bin' >> /etc/profile

source /etc/profile
```

之后直接输入 `mongod` 就可以运行了。
