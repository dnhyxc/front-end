...
> 上述代码中，`*(ptrArray[i])` 首先通过 `ptrArray[i]` 获取到指针数组中存储的三个指针变量，在通过 `*(ptrArray[i])` 解应用地址得到指针地址分别指向的变量 10、20、30。
...
> 上述代码中之所以能直接通过 `parr[i][j]` 得到指针所指向的变量，是因为 `parr[i][j]` 就等价于 `*(*(parr + i) + j)`，即这里的 `[]` 就等价于解应用。
