...
// watch(keyword, (value) => {
//   // 防止改变编辑器内容时光标重定向
//   console.log(monacoEditor, "monacoEditor?.getValue()");
...
onMounted(() => {
  monacoEditor = monaco.editor.create(document.getElementById("CODE_EDITOR"), {
    value: keyword.value,
    language: "javascript",
    theme: "vs-dark",
    ...
    columnSelection: true, // 启用列编辑 按下shift键位然后按↑↓键位可以实现列选择然后实现列编辑
  ...
  console.log(monacoEditor, "monacoEditor", monacoEditor.getAction());

  bindKeyWithAction(
    monacoEditor,
    monaco.KeyMod.CtrlAlt | monaco.KeyCode.US_OPEN_SQUARE_BRACKET,
    "editor.action.jumpToBracket"
  );
});

function bindKeyWithAction(editor, key, actionID) {
  monacoEditor.addCommand(key, function () {
    monacoEditor.trigger("", actionID);
  });
}
</script>
